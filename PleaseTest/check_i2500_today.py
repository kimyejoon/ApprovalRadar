...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        # Plain
        url_plain = f"http://openapi.foodsafetykorea.go.kr/api/{key}/I2500/json/1/10/CHNG_DT=20260521"
        ...
        total_p = js_plain.get("total_count", "0")
        
        # Live
        url_live = f"http://openapi.foodsafetykorea.go.kr/api/{key}/I2500/json/1/10/CHNG_DT=20260521&SYS_SYNC=LIVE"
        ...
        total_l = js_live.get("total_count", "0")
        
        print("=== I2500 CHNG_DT=20260521 ===")
        print(f"Plain | Code: {code_p} ({msg_p}) | Total: {total_p}")
        print(f"Live  | Code: {code_l} ({msg_l}) | Total: {total_l}")
        
        if total_l != "0":
            rows = js_live.get("row", [])
            print("Live First 5 rows:")
            for idx, r in enumerate(rows[:5]):
                print(f"  {idx+1}: {r.get('BSSH_NM')} | {r.get('LCNS_NO')} | {r.get('PRSDNT_NM')}")
