...
async def run_test(client, key, name, query_part):
    url = f"http://openapi.foodsafetykorea.go.kr/api/{key}/I2861/json/1/1000/{query_part}"
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"[{name}] Code: {code} | Total: {total_count:5s} | Returned rows: {len(rows)} | First Row: {first_nm} ({first_dt})")
        else:
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        await run_test(client, key, "4. CHNG_DT=20260520", "CHNG_DT=20260520")
        await run_test(client, key, "5. CHNG_DT=20260521", "CHNG_DT=20260521")
