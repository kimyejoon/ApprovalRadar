...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        # Fetch I2861 with CHNG_DT=20260521
        res = await client.get(f"http://openapi.foodsafetykorea.go.kr/api/{key}/I2861/json/1/1000/CHNG_DT=20260521")
        if res.status_code == 200:
            js = res.json()
            ...
            for idx, r in enumerate(today_rows[:10]):
                print(f"  {idx+1}: {r.get('BSSH_NM')} | {r.get('LCNS_NO')}")
        else:
