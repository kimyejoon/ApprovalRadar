...
```
P1 ~ P678:  일반음식점  [가→힘→특수문자/숫자/영문]
P679 ~ ?:   휴게음식점  [(→숫자→영문→가나다순]
 ?  ~ ?:   제과점영업
 ?  ~ ?:   위탁급식영업
 ?  ~ ?:   집단급식소
 ?  ~ ?:   유흥주점/단란주점
...
| **1시간 보장** | ✅ 영향 없음 | 모든 953p를 1시간 내 전체 스캔 |
| **신규 레코드 위치** | ⚠️ 중간 삽입 가능 | 새 일반음식점("나") → P~200 삽입, 후속 페이지 내용이 밀림 |
| **Tail Probe** | ✅ 유효 | 새 업종 그룹이 추가되거나 마지막 그룹 증가 시 tail 확장 감지 |
| **page_labels 의미** | ✅ 개선됨 | 업종 정보 함께 저장 — `일반:가~나` vs `휴게:가~나` 구분 가능 |

### 페이지 경계 이동 현상
새 레코드가 DB 중간에 삽입되면 API 서버가 전체를 재정렬·재페이지합니다. 예를 들어, 새 `일반음식점` 업소("나가자")가 추가되면 P200 이후의 모든 페이지 내용이 1건씩 밀립니다. Oldest-First Scan이 모든 페이지를 1시간 내에 재스캔하므로 이 변동도 정확히 포착됩니다.
