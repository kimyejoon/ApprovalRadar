...
- 완료 로그 형식: `[I2861] ✅ P20/953 [일반:가~가] 완료 | 조회 1,000건 | ...`
...
## 🗂️ I2861 DB 정렬 구조 (발견된 사실)

식품안전나라 `I2861` 서비스는 단순 상호명 순이 아닌 **2단계 정렬** 구조를 가지고 있습니다.

### 정렬 키: `INDUTY_NM(업종) → BSSH_NM(상호명)`

```
P1   ~ P678:  일반음식점  [가→힘→특수문자/숫자/영문]   ← 수집 대상
P679 ~ P898:  휴게음식점  [(→숫자→영문→가나다순]       ← 수집 대상
P899 ~ P913:  제과점영업                              ← 수집 대상
─────────────────────────── target_boundary ─────────
P914 ~ P927:  위탁급식영업                            ← 수집 불필요 (스킵)
P928 ~ P935:  집단급식소                              ← 수집 불필요 (스킵)
P936 ~ P953:  유흥주점영업 / 단란주점                  ← 수집 불필요 (스킵)
```

### 업종 내 상호명 정렬 순서
업종 내에서는 **한국 DB 로케일** 기준으로 정렬됩니다:
1. 한글 (가나다순, 가~힘)
2. 특수문자 (`(`, `[`, 등)
3. 숫자 (`0~9`)
4. 영문 (`A~z`)

### 업종 인식 스캔 (Industry-Aware Scanning)

수집이 필요 없는 업종 페이지를 스캔에서 제외하고, 경계가 이동했을 때 자동으로 감지하는 **2단계 구조**를 사용합니다.

```python
TARGET_INDUSTRIES = {"일반음식점", "휴게음식점", "제과점영업"}  # 수집 대상
SKIP_INDUSTRIES   = {"위탁급식영업", "집단급식소", "유흥주점영업", "단란주점"}  # 스킵
```

**스캔 후보 풀:**
```
candidate_set = known_target_pages ∪ unknown_pages  (known_skip_pages 제외)
```

| 페이지 종류 | 처리 방식 |
|---|---|
| 타깃 업종 확인 (일반/휴게/제과) | 정상 스캔 + scraper 파이프라인 실행 |
| 스킵 업종 확인 (위탁/급식/주점) | 스캔 후보 제외 (API 호출 없음) |
| 업종 미확인 (첫 사이클, 신규 페이지) | 스캔 후보 포함 → 스캔 중 업종 자동 파악 |

**자가 교정 흐름:**
```
초기 (page_industries 비어있음):
  unknown = 전체 953p → 기존과 동일 풀스캔, INDUTY_NM 자동 축적

2회차 이후:
  candidate_set ≈ 938p (known_skip ~15p 제외, API 절감)
```

### 업종 경계 프로브 (Industry Boundary Probe)

`target_boundary` (마지막 타깃 업종 페이지) 이후를 **한 페이지씩 순차 확인**하여 경계 이동을 정확하게 감지합니다. 메인 스캔 완료 후 매 사이클 실행됩니다.

```
매 사이클 메인 스캔 완료 후:
  P(target_boundary + 1) 확인
    └─ 타깃 업종 → target_boundary += 1, 다음 페이지 계속 확인
    └─ 스킵 업종 → 즉시 STOP (경계 확정)
  최대 INDUSTRY_BOUNDARY_PROBE_MAX(=5)장까지 탐색
```

**경계 이동 감지 시나리오 (수개월 후 일반음식점 1,000건 추가):**
```
기존: [P913 = 제과점영업] [P914 = 위탁급식영업]
추가 후: [P914 = 제과점영업] [P915 = 위탁급식영업]

경계 프로브:
  P914 확인 → "제과점영업" → 타깃! target_boundary P913 → P914로 확장
  P915 확인 → "위탁급식영업" → STOP
  → "📍 target_boundary 갱신: P913 → P914"
```

**로그 예시:**
```
[I2861] 🚀 Oldest First Scan 시작 | 후보 938p (⏭ 업종 스킵 15p 제외) | ...
[I2861] 🔬 업종 경계 프로브 P914/953 업종 확인 중...
[I2861] 🔬 P914 [위탁급식영업] → 스킵 업종. 경계 확정: 마지막 타깃 P913
[I2861] 📍 업종 경계 유지: 타깃 마지막 P913 확정
```
...
| 항목 | 영향 | 이유 |
|---|---|---|
| **변동 감지 정확성** | ✅ 영향 없음 | `CHNG_DT` 필드로 감지, 위치 무관 |
| **1시간 보장** | ✅ 유지 | 타깃 업종 전체 (~938p)를 1시간 내 스캔 |
| **API 절감** | ✅ ~15p 절약 | 스킵 업종 페이지 API 호출 제거 |
| **경계 이동 감지** | ✅ 자동 | 매 사이클 경계 프로브 (+최대 5p) |
| **신규 레코드 위치** | ⚠️ 중간 삽입 가능 | 타깃 페이지 전체를 1시간 내 재스캔하므로 자동 해결 |

### 페이지 경계 이동 현상
새 레코드가 DB 중간에 삽입되면 API 서버가 전체를 재정렬·재페이지합니다. 예를 들어, 새 `일반음식점` 업소("나가자")가 추가되면 P200 이후의 모든 페이지 내용이 1건씩 밀립니다. Oldest-First Scan이 모든 타깃 페이지를 1시간 내에 재스캔하므로 이 변동도 정확히 포착됩니다.
