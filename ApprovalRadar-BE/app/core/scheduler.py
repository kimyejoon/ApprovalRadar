...
def start_scheduler():
    ...
    # DB 최적화 (일요일 새벽 3시)
    from database import vacuum_db, backup_db, prune_db, prune_log_files
    ...
    # DB 오래된 데이터 및 로그 파일 자동 정리 (매일 새벽 3시 30분)
    def _scheduled_maintenance():
        prune_log_files(keep_days=30)
        prune_db(days=7)
    scheduler.add_job(_scheduled_maintenance, 'cron', hour=3, minute=30, id="db_prune_job")
