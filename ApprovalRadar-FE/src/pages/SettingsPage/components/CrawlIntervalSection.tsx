...
import { useCrawlIntervalQuery, useUpdateCrawlIntervalMutation } from '../api';

const PRESET_MINUTES = [5, 10, 15, 30, 60, 120] as const;

export function CrawlIntervalSection() {
  const { data: currentInterval, isLoading } = useCrawlIntervalQuery();
  const updateMutation = useUpdateCrawlIntervalMutation();
  const [localVal, setLocalVal] = useState<number>(30);
    ...
    <Card className="border-border-standard bg-surface">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {PRESET_MINUTES.map((m) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${localVal === m
        ...
        <button
          ...
          onClick={handleSave}
          disabled={updateMutation.isPending || localVal === currentInterval}
        ...
        >
          ...
          {updateMutation.isPending ? '적용 중...' : '주기 적용'}
        </button>
